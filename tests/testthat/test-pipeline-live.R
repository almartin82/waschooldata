# ==============================================================================
# LIVE Pipeline Tests for waschooldata
# ==============================================================================
#
...
test_that("Washington DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Washington DOE URL and verify HTTP 200")
})
...
test_that("Can download Washington enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Washington enrollment file", {
  skip_if_offline()
...
test_that("waschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- waschooldata::get_available_years()
  ...
  tryCatch({
    raw <- waschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- waschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- waschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- waschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- waschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- waschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- waschooldata:::get_cache_path(2024, "enrollment")
