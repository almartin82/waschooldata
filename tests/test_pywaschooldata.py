...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = wa.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pywaschooldata as wa
        assert hasattr(wa, 'fetch_enr')
        assert hasattr(wa, 'fetch_enr_multi')
        assert hasattr(wa, 'tidy_enr')
        assert hasattr(wa, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pywaschooldata as wa
        assert hasattr(wa, '__version__')
        assert isinstance(wa.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = wa.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = wa.fetch_enr(years['max_year'])
        # Total enrollment in Washington varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Washington should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = wa.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = wa.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pywaschooldata as wa
        years = get_test_years()
        df_single = wa.fetch_enr(years['max_year'])
        df_multi = wa.fetch_enr_multi([years['max_year']])
