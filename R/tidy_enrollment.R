...
tidy_enr <- function(df) {
  ...
  invariants <- c(
    ...
    "campus_id", "school_code", "campus_name",
    "school_type", "esd_id", "esd_name", "county"
  )
  ...
  # Gender columns
  gender_cols <- c("male", "female", "gender_x")
...
id_enr_aggs <- function(df) {
  df |>
    dplyr::mutate(
      ...
      # Campus level: Type == "Campus" or "School"
      is_campus = type %in% c("Campus", "School")
    )
}
...
enr_grade_aggs <- function(df) {
  ...
  # K-8 aggregate (elementary + middle school)
  k8_agg <- df |>
    dplyr::filter(
      subgroup == "total_enrollment",
  ...
  dplyr::bind_rows(elem_agg, middle_agg, k8_agg, hs_agg, k12_agg)
}
