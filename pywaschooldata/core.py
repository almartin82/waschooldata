"""
Core functions wrapping waschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("waschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch Washington school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch Washington school enrollment data for multiple years.
    ...
    --------
    >>> import pywaschooldata as wa
    >>> df = wa.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
